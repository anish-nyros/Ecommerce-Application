...
export const Details = (props) => {
	...
	useEffect(() => { 
		console.log("printing data inside details page ",datas)
		console.log("printing id inside details page ",itemId)
	},[])
	...
	const handleCart = (item) => {
	  ...
	  useEffect(() => {
		let carData = localStorage.setItem("Items", JSON.stringify(cart))
	  },[cart])
	  console.log("locala storage data in details page" ,cart)
  return (
    <div>
        <Navbar />
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
				<aside>
					...
					<div class="border border-gray-200 shadow-sm p-3 text-center rounded mb-5">
						<img class="object-cover inline-block" width="400" src={datas.image} alt={datas.id}/>
					</div>
				...
				<main>
					...
					<div class="flex flex-wrap items-center space-x-2 mb-2">
						...
						<img class="d-inline-block h-4" src={datas.image} alt="Rating"/>
						...
						<span class="text-gray-400">
							<i class="fa fa-shopping-bag mr-2"></i>{datas.rating.count}
						</span>
					...
					<p class="mb-4 font-semibold text-xl"> $ {datas.price}
